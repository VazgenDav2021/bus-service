...
driverRouter.get('/me', async (req: AuthRequest, res, next) => {
  try {
...
driverRouter.get('/scan/student', async (req: AuthRequest, res, next) => {
  try {
    ...
    if (!parsed.success) {
      sendError(
        res,
        400,
        parsed.error.errors[0]?.message ?? ERROR_MESSAGES.qrTokenRequired,
...
driverRouter.post('/scan/board', async (req: AuthRequest, res, next) => {
  try {
