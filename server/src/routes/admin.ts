...
function getUploadedStudentImagePath(filename?: string): string | undefined {
  return filename ? `students/${filename}` : undefined;
}

function withStudentImageCleanup(
  ...
  return asyncHandler(async (req, res, next) => {
    try {
      ...
      const imagePath = getUploadedStudentImagePath((req as { file?: { filename?: string } }).file?.filename);
      if (imagePath) {
...
adminRouter.post(
  ...
  withStudentImageCleanup(async (req, res) => {
