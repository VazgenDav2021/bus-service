...
import { createDriverSchema, paginatedListQuerySchema, } from '../modules/busOwner/busOwnerValidation.js';
import { parseOrThrow } from '../utils/validation.js';
import { asyncHandler } from '../shared/http/asyncHandler.js';
export const busOwnerRouter = Router();
busOwnerRouter.use(authenticate, requireRole('BUS_OWNER'));
busOwnerRouter.get('/drivers', asyncHandler(async (req, res) => {
...
busOwnerRouter.post('/drivers', asyncHandler(async (req, res) => {
    const ownerId = req.user.sub;
...
busOwnerRouter.get('/drivers/:driverId/scans', asyncHandler(async (req, res) => {
    const ownerId = req.user.sub;
    const query = paginatedListQuerySchema.parse(req.query);
    const result = await busOwnerService.getOwnerDriverScans(ownerId, req.params.driverId, query);
    res.json(result);
}));
