...
export async function getBusOwnerStats(ownerId, _from, _to) {
...
export async function getOwnerBuses(ownerId, query) {
    const pagination = {
        page: query.page ?? 1,
...
export async function createOwnerBus(ownerId, data) {
...
export async function getOwnerDrivers(ownerId, query) {
    const pagination = {
        page: query.page ?? 1,
        pageSize: query.pageSize ?? 10,
    };
...
export async function createOwnerDriver(ownerId, data) {
...
export async function getOwnerAssignments(ownerId, query) {
    const pagination = {
        page: query.page ?? 1,
        pageSize: query.pageSize ?? 10,
    };
...
export async function createOwnerAssignment(ownerId, data) {
    const startDate = normalizeStartOfDayUtc(data.startDate);
    const endDate = normalizeEndOfDayUtc(data.endDate);
    if (endDate < startDate) {
        throw new AppError(400, ERROR_MESSAGES.endDateMustBeAfterStartDate, ERROR_CODES.invalidDateRange);
    }
    ...
    if (overlap) {
        throw new AppError(400, ERROR_MESSAGES.assignmentOverlap, ERROR_CODES.assignmentOverlap);
    }
